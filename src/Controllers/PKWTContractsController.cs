...
[Route("[controller]")]
[Authorize]
public class PKWTContractsController : ControllerBase
{
    private readonly IServiceAsync<PKWTContracts> _service;

    public PKWTContractsController(IServiceAsync<PKWTContracts> service)
    {
        _service = service;
    }
    ...
    public async Task<IActionResult> GetById(int id)
    {
        try
        {
            ...
            var response = new Response<PKWTContracts>(result);
            return Ok(response);
        }
        catch (Exception ex)
        {
            ...
            Trace.WriteLine(message, "PKWTContractsController");
            return BadRequest(new { message });
        }
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] PKWTContracts obj)
    {
        try
        {
    ...
    [HttpPut]
    public async Task<IActionResult> Edit([FromBody] PKWTContracts obj)
    {
        try
        {
    ...
    public async Task<IActionResult> UploadFile(IFormFile file)
    {
        try
        {
            ...
            var baseFolderPath = Path.Combine(Directory.GetCurrentDirectory(), "ContractsFiles");
            ...
            var relativeFilePath = Path.Combine("ContractsFiles", datePath, customFileName).Replace("\\", "/");
