...
{
    private readonly IServiceAsync<Users> _service;

    public UsersController(IServiceAsync<Users> service)
    {
        _service = service;
    }
    ...
    public async Task<IActionResult> GetById(int id)
    {
        try
        {
            ...
            var response = new Response<Users>(result);
            return Ok(response);
        }
        catch (Exception ex)
        {
            ...
            Trace.WriteLine(message, "UsersController");
            return BadRequest(new { message });
        }
    }
