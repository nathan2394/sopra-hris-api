...
namespace sopra_hris_api.src.Services.API
{
    public class EmployeeDetailService : IServiceAsync<EmployeeDetails>
    {
                ...
                if (!string.IsNullOrEmpty(filter))
                {
                    ...
                    foreach (var f in filterList)
                    {
                        ...
                        if (searchList.Length == 2)
                        {
                            ...
                            {
                                "employee" => query.Where(x => x.EmployeeID.ToString().Equals(value)),
                ...
                if (!string.IsNullOrEmpty(sort))
                {
                    ...
                    if (temp.Length > 1)
                    {
                        query = orderBy.ToLower() switch
                        {
                            ...
                            "id" => query.OrderByDescending(x => x.EmployeeDetailID),
                            _ => query
                        };
                    }
                    else
                    {
                        query = orderBy.ToLower() switch
                        {
                            ...
                            "id" => query.OrderBy(x => x.EmployeeDetailID),
                            _ => query
                        };
                    }
                }
        ...
        public async Task<EmployeeDetails> GetByIdAsync(long id)
        {
            try
            {
                return await (from a in _context.EmployeeDetails
                              join ad in _context.AllowanceDeduction
                              on a.AllowanceDeductionID equals ad.AllowanceDeductionID
                              where a.EmployeeDetailID == id && a.IsDeleted == false
                              select new EmployeeDetails
                              {
                                  EmployeeDetailID = a.EmployeeDetailID,
                                  EmployeeID = a.EmployeeID,
