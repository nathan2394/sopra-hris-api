...
namespace sopra_hris_api.src.Services.API
{
    public class DivisionService : IServiceAsync<Division>
    {
        ...
        public DivisionService(EFContext context)
        {
            _context = context;
        }
        ...
        public async Task<Division> CreateAsync(Division data)
        {
            ...
            try
            {
                await _context.Divisions.AddAsync(data);
        ...
        public async Task<bool> DeleteAsync(long id, long UserID)
        {
            ...
            try
            {
                var obj = await _context.Divisions.FirstOrDefaultAsync(x => x.DivisionID == id && x.IsDeleted == false);
        ...
        public async Task<Division> EditAsync(Division data)
        {
            ...
            try
            {
                var obj = await _context.Divisions.FirstOrDefaultAsync(x => x.DivisionID == data.DivisionID && x.IsDeleted == false);
        ...
        public async Task<ListResponse<Division>> GetAllAsync(int limit, int page, int total, string search, string sort, string filter, string date)
        {
            try
            {
                ...
                var query = from a in _context.Divisions where a.IsDeleted == false select a;
        ...
        public async Task<Division> GetByIdAsync(long id)
        {
            try
            {
                return await _context.Divisions.AsNoTracking().FirstOrDefaultAsync(x => x.DivisionID == id && x.IsDeleted == false);
            }
            catch (Exception ex)
            {
                Trace.WriteLine(ex.Message);
