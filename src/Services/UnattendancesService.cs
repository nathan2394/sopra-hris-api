...
DROP TABLE #TEMP", new SqlParameter("VoucherNo", data.VoucherNo)).ToListAsync();

                if (mailto?.Count > 0)
                {
                    var ovt = await _context.Unattendances.FirstOrDefaultAsync(x => x.VoucherNo == data.VoucherNo && x.IsDeleted == false);
